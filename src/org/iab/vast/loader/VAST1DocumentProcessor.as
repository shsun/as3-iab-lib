package org.iab.vast.loader {
	...
	import org.iab.vast.parser.VAST1Parser;
	...
	internal class VAST1DocumentProcessor extends EventDispatcher {
		...
		private function needLoadVASTWrapper(ad : VASTAd) : Boolean {
			return ad.wrapperAd != null && (  maxNumWrapperRedirects == -1 || maxNumWrapperRedirects > 0
			);
		}
		...
		private function loadVASTWrapper(ad : VASTAd, completionCallback : Function) : void {
			...
			var wrapperLoader : VASTLoader = new VASTLoader(Math.max(-1, maxNumWrapperRedirects - 1), httpLoader)
			;

			var wrapperResource : URLResource = new URLResource(ad.wrapperAd.vastAdTagURL);
