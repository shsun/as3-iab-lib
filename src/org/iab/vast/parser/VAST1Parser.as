package org.iab.vast.parser {
	import org.iab.vast.model.VASTAd;
	import org.iab.vast.model.VASTAdBase;
	import org.iab.vast.model.VASTAdPackageBase;
	import org.iab.vast.model.VASTCompanionAd;
	import org.iab.vast.model.VASTDocument;
	import org.iab.vast.model.VASTInlineAd;
	import org.iab.vast.model.VASTMediaFile;
	import org.iab.vast.model.VASTNonLinearAd;
	import org.iab.vast.model.VASTResourceType;
	import org.iab.vast.model.VASTTrackingEvent;
	import org.iab.vast.model.VASTTrackingEventType;
	import org.iab.vast.model.VASTUrl;
	import org.iab.vast.model.VASTVideo;
	import org.iab.vast.model.VASTVideoClick;
	import org.iab.vast.model.VASTWrapperAd;
	 ...
	 */
	public class VAST1Parser {
		/**
		 * Constructor.
		 */
		public function VAST1Parser() {
			super();
		}
					...
					case NODEKIND_ELEMENT:
						{
							switch (child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseAdTag() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch (child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseInlineTag() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch (child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseWrapperTag() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								case TRACKING:
									{
										...
										var trackingEvent : VASTTrackingEvent = new VASTTrackingEvent(VASTTrackingEventType.fromString(eventType));
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseTrackingEvents() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
		...
		private function parseURLTags(xml : XML, maxNumURLs : int = -1) : Vector.<VASTUrl> {
			...
			for (var i : int = 0; i < children.length(); i++) {
				...
				if (maxNumURLs >= 0 && urls.length >= maxNumURLs) {
					break;
				}
					...
					case NODEKIND_ELEMENT:
						{
							switch (child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseURLTags() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseVideo() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseMediaFiles() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseInlineCompanionAds() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseWrapperCompanionAds() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseWrapperNonLinearAds() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseInlineNonLinearAds() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseExtensions() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
					...
					case NODEKIND_ELEMENT:
						{
							switch(child.localName()) {
								...
								default:
									CONFIG::LOGGING {
										logger.debug("[VAST] parseVideoClicks() - Unsupported VAST tag: " + child.localName());
									}
									break;
							}
							break;
		...
		private function validate(document : VASTDocument) : Boolean {
			...
			for each (var ad:VASTAd in document.ads) {
				...
				if (inlineAd != null && wrapperAd != null) {
					CONFIG::LOGGING {
						logger.debug("[VAST] Invalid VAST document:  Ad cannot have both Inline and Wrapper");
					}
				...
				if (inlineAd != null) {
					if (inlineAd.adSystem == null || inlineAd.adTitle == null || inlineAd.impressions.length == 0) {
						CONFIG::LOGGING {
							logger.debug("[VAST] Invalid VAST document:  InlineAd requires AdSystem, AdTitle, and Impression");
						}
					...
					if (video != null) {
						if (video.duration == null) {
							CONFIG::LOGGING {
								logger.debug("[VAST] [VAST] Invalid VAST document:  Video requires Duration");
							}
						...
						for each (var mediaFile:VASTMediaFile in video.mediaFiles) {
							if (mediaFile.url == null || mediaFile.delivery == null || mediaFile.bitrate == 0 || mediaFile.width == 0 || mediaFile.height == 0) {
								CONFIG::LOGGING {
									logger.debug("[VAST] Invalid VAST document:  MediaFile requires URL, plus delivery, bitrate, width and height attributes");
								}
					...
					for each (var companionAd:VASTCompanionAd in inlineAd.companionAds) {
						if (validateAdBase(companionAd) == false) {
							CONFIG::LOGGING {
								logger.debug("[VAST] Invalid VAST document:  CompanionAd requires width, height, and resourceType attributes");
							}
					...
					for each (var nonLinearAd:VASTNonLinearAd in inlineAd.nonLinearAds) {
						if (validateAdBase(nonLinearAd) == false) {
							CONFIG::LOGGING {
								logger.debug("[VAST] Invalid VAST document:  NonLinearAd requires width, height, and resourceType attributes");
							}
				...
				if (wrapperAd != null) {
					if (wrapperAd.adSystem == null || wrapperAd.vastAdTagURL == null || wrapperAd.impressions.length == 0) {
						CONFIG::LOGGING {
							logger.debug("[VAST] Invalid VAST document:  WrapperAd requires AdSystem, VASTAdTagURL, and Impression");
						}
		...
		private function validateTrackingEvents(trackingEvents : Vector.<VASTTrackingEvent>) : Boolean {
			...
			for each (var event:VASTTrackingEvent in trackingEvents) {
				if (event.type == null) {
					CONFIG::LOGGING {
						logger.debug("[VAST] Invalid VAST document:  TrackingEvent requires valid type attribute");
					}
		...
		private function validateAdBase(adBase : VASTAdBase) : Boolean {
			if (adBase.width == 0 || adBase.height == 0 || adBase.resourceType == null) {
				return false;
			}
		...
		CONFIG::LOGGING
		private static const logger : Logger = Log.getLogger("org.osmf.vast.parser.VAST1Parser");
	}
}
